...
module.exports = function (options) {
  ...
  function hook_listen_redis (args, done) {
    ...
    listen_topics(seneca, args, listen_options, function (topic) {
      seneca.log.debug('listen', 'subscribe', topic + '_act', _.omit(listen_options, 'password'), seneca)
  ...
  function hook_client_redis (args, clientdone) {
    ...
    function make_send (spec, topic, send_done) {
      ...
      send_done(null, function (args, done) {
      	...
      	!args.meta$ && (args.$meta = {})
	      args.meta$.sync = false
	      // _.set(args, 'meta$.sync', false)  when update to latest lodash
  ...
  function handle_events (redisclient) {
    ...
    redisclient.on('ready', function () {
      redisclient.on('error', function (err) {
        seneca.log.error('transport', 'redis-async', err)
      })
    })
  }
